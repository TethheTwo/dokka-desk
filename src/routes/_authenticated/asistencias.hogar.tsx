...
export const Route = createFileRoute("/_authenticated/asistencias/hogar")({
  head: () => ({
    meta: [
      { title: "Asistencia Hogar — DOKKA Desk" },
      { name: "description", content: "Registrar nueva asistencia para hogar." },
    ],
  }),
  component: AsistenciaHogarPage,
});

function AsistenciaHogarPage() {
  return (
    <div className="min-h-screen bg-[var(--app-bg)] text-foreground">
      <AppTopBar />
      <PageHeader title="Asistencia Hogar" subtitle="Registrar nueva asistencia para el hogar." />
...
function AsistenciaCard() {
  ...
  const resetForm = () => {
    setForm({ vigente: "", detalleProblema: "" });
    setAttachments([]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const extras: { label: string; value: string }[] = [
      { label: "Detalle del problema", value: (form.detalleProblema ?? "").trim() },
    ...
    <div className="bg-card rounded-md shadow-sm border border-border overflow-hidden">
      ...
      <div className="px-6 py-5 border-b border-border flex items-center justify-between gap-3">
        <h2 className="text-sm font-semibold text-foreground">
          Registrar nueva asistencia para hogar
        </h2>
        <button
          type="button"
      ...
      <form onSubmit={handleSubmit} className="px-6 py-6 space-y-5">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          ...
          <Field label="Detalle del problema" required>
            <textarea
              value={form.detalleProblema ?? ""}
              onChange={(e) => set("detalleProblema", e.target.value)}
          ...
          <Field label="Dirección donde se asiste" required>
            <input
              type="text"
